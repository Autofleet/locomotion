...
const RidePageContextProvider = ({navigation, children}) => {
  ...
  const [disableAutoLocationFocus, setDisableAutoLocationFocus] =
    useState(false);
  ...
  const loadActiveRide = async () => {
    ...
    if (activeRideState) {
      const rideSummary = await getRideSummary({
      ...
      if (rideSummary && rideSummary.state === 'completed') {
        ...
        const duration = moment(dropoffTime).diff(
          moment(pickupTime),
          'minutes',
        );
  ...
  if (Config.STATIONS_REFRESH_RATE) {
    useInterval(() => {
      if (
        !rideOffer &&
        (!requestStopPoints.pickup || !requestStopPoints.dropoff)
      ) {
        getStations();
      }
  ...
  const calculatePickupEta = origin => {
    if (origin.completedAt) {
      ...
      setDisplayMatchInfo(
        etaDiff <= useSettings.settingsList.ARRIVE_REMINDER_MIN ||
          (activeRideState && activeRideState.arrivingPush !== null),
      );
    }
  };
