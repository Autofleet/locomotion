...
const TimeItem = styled(Button)`
  ...
  background-color: ${({selected, theme}) =>
    selected ? theme.primaryColor : theme.pageBackgroundColor};
...
const OrderTimeSelector = ({selected, text, onPress, side, disabled}) => (
  <TimeItem
    selected={selected}
    onPress={onPress}
    side={side}
    disabled={disabled}
    data-test-id={`${text}OrderTimeButton`}>
...
export default ({onScheduleTimeSelect, disableFuture = false, scheduledTo}) => {
  ...
  return (
    <Container>
      ...
      {scheduleType === 'future' ? (
        <DateTimePicker onScheduleTimeSelect={onScheduleTimeSelect} />
      ) : null}
    </Container>
  );
};
