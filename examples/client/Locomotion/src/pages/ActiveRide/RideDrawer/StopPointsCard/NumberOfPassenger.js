...
const PassengerAmountIcon = styled.Image.attrs({
  source: PassengerAmountIconSource,
})`
  width: 16px;
  height: 16px;
`;
...
export default ({amount, onChange}) => {
  ...
  const validateChange = (validationAmount, change) =>
    MAX_NUMBER_OF_PASSENGER >= validationAmount + change &&
    validationAmount + change >= MIN_NUMBER_OF_PASSENGER;
      ...
      <View style={{width: '100%', flexDirection: 'row'}}>
        ...
        <PassengerControlersContainer>
          <SetPassengerAmountBoxContainer
            disabled={!validateChange(newAmount, -1)}>
          ...
          <SetPassengerAmountBoxContainer
            disabled={!validateChange(newAmount, 1)}>
            <SetPassengerAmountBox
              data-test-id="SetPassengerAmountPlusButton"
              onPress={getNewAmountFunction(1)}>
              <PlusIcon />
            </SetPassengerAmountBox>
          </SetPassengerAmountBoxContainer>
