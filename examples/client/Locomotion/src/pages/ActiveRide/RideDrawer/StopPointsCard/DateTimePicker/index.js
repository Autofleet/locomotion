...
const DateTimePicker = ({onScheduleTimeSelect}) => {
  ...
  const getOptionalDates = () => {
    ...
    const min = moment().add(
      useSettings.settingsList.FUTURE_ORDER_MIN_TIME,
      'minutes',
    );
    const max = moment(min).add(
      useSettings.settingsList.FUTURE_ORDER_MAX_TIME,
      'hours',
    );
    ...
    while (calculatedDate.diff(max) < 0) {
      ...
      if (workingHours[dayInWeek]) {
        ...
        const timeData = getOptionalTimes(
          date.format('DD.MM.YYYY'),
          workingHours[dayInWeek],
        );
        const filteredTimeData = timeData.filter(time =>
          moment(time).isSameOrAfter(min),
        );
