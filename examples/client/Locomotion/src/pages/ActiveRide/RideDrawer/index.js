...
const getRideState = activeRide => {
  // false, driverOnTheWay, driverArrived, onBoard
  if (!activeRide) {
    return false;
  }
...
const RideDrawer = ({
  ...
  const buttonAction = async () => {
    ...
    if (
      ((!rideOffer && readyToBook) || (rideOffer && offerExpired)) &&
      !requestStopPoints.scheduledTo
    ) {
      return createOffer();
    }
  ...
  return (
    <DrawerContainer>
      ...
      />
      {!allowRideOrder ? (
        <AddPaymentBar onPress={() => navigation.navigate(MAIN_ROUTES.PAYMENT)}>
          {I18n.t('payments.setPaymentBanner')}
        </AddPaymentBar>
      ) : null}
      <FutureRides
        futureRides={futureRides}
        ...
        {!isPopupOpen('ridePopupsStatus') && !futureOrdersState ? (
          <>
            ...
            (pickupEta <= useSettings.settingsList.ARRIVE_REMINDER_MIN ||
              (activeRide && activeRide.arrivingPush !== null) ||
              ...
              <>
                <StopPointsEtaCard
                  ...
                  dropoffEtaDrift={
                    useSettings.settingsList.DISPLAY_MAX_ETA_DRIFT
                  }
                />
                <DrawerButtonContainer>
                  <RoundedButton
            ...
            {!rideState && rideOffer ? (
              <>
                <OfferCard
                  ...
                  rideOffer={rideOffer}
                  etaMediumThreshold={
                    useSettings.settingsList.ETA_MEDIUM_THRESHOLD
                  }
          ...
          <RideButtonContainer>
            {!futureOrdersState && (
              <RoundedButton
                ...
                {I18n.t(
                  rideState ? 'home.cancelRideButton' : 'home.letsRideButton',
                )}
              </RoundedButton>
            )}
          </RideButtonContainer>
