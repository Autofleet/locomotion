...
import {VehicleDot, MapButtonsContainer} from './styled';
...
export default ({mapSettings}) => {
  ...
  const showsUserLocation =
    !activeRideState ||
    (activeRideState && !activeRideState.stopPoints[0].completedAt);

  const focusMarkers = () => {
    if (!activeRideState) {
      return;
    }
    const activeSp = activeRideState.stopPoints.find(
      sp => sp.state === 'pending' || sp.state === 'arrived',
    );

    const additional = [];
    if (
      activeRideState &&
      activeSp.type === 'pickup' &&
      !activeSp.completedAt &&
      mapRegion.latitude &&
      mapRegion.longitude
    ) {
      additional.push({
    ...
    if (
      activeRideState.vehicle &&
      activeRideState.vehicle.location &&
      displayMatchInfo
    ) {
      additional.push({
    ...
    mapInstance.current.fitToCoordinates(
      [
        {
          latitude: parseFloat(activeSp.lat),
          longitude: parseFloat(activeSp.lng),
        },
        ...additional,
      ],
      {
  ...
  const VehicleMarker = () => {
    if (
      ...
      if (activeSpState && activeSpState.polyline) {
        ...
        newPoint = ruler.pointOnLine(line, [fixLng, fixLat]).point;
      } else {
    ...
    <>
      <MapView
        ...
        onUserLocationChange={event => {
          if (
            (Platform.OS === 'ios' && !Config.MAP_PROVIDER !== 'google') ||
            !showsUserLocation ||
            disableAutoLocationFocus
          ) {
        ...
        {activeSpState ? (
          <StationsMap
            isInOffer={!!rideOffer}
            markersMap={
              activeRideState && activeRideState.stopPoints
                ? activeRideState.stopPoints.map(m => ({
