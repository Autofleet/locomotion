    ...
    <Modal isVisible={isPopupOpen('futureRideCanceled') || false}>
      <Container>
        <CloseContainer
          noBg
          onPress={() => closePopup()}
          data-test-id="CloseFutureRideCanceledPopup">
          <ResetInputIcon />
        </CloseContainer>
        <View style={{flex: 2, textAlign: 'left', maxWidth: '80%'}}>
          <Title>{i18n.t('popups.futureRideCanceled.title')}</Title>
          <SubTitle>{i18n.t('popups.futureRideCanceled.subTitle')}</SubTitle>
        </View>
      </Container>
    </Modal>
  );
