...
const usePayments = () => {
  ...
  const getCustomer = async () => {
    const {data: clientData} = await network.get(
      '/api/v1/me/payments/customer',
    );
    return clientData;
  };
  ...
  const createCustomer = async () => {
    const {data: clientData} = await network.post(
      '/api/v1/me/payments/customer',
    );
  ...
  const getPaymentMethods = async () => {
    const {data: paymentMethodsData} = await network.get(
      '/api/v1/me/payments/methods',
    );
  ...
  const detachPaymentMethod = async paymentMethodId => {
    const {data: paymentMethodsData} = await network.post(
      '/api/v1/me/payments/detach',
      {paymentMethodId},
    );
    return paymentMethodsData;
  };
