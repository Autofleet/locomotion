...
const formatResponseLog = function ({data = ''}) {
  let str = (typeof data === 'string' ? data : JSON.stringify(data)).slice(
    0,
    limit,
  );
  if (str.length === limit) {
...
class Network {
  ...
  constructor(settings = {}) {
    ...
    this.axios.interceptors.request.use(request => {
      try {
        Mixpanel.setEvent('Network request', {
    ...
    this.axios.interceptors.response.use(
      response => {
        try {
          Mixpanel.setEvent('Network response', {
      ...
      error => {
        try {
          console.error(
            `Request rejected [${error.config.method}] ${error.config.url}: ${error}`,
            formatResponseLog(error.response),
          );
        } catch (e) {
    ...
    HTTPMethods.map(method => {
      this[method] = async (...args) => {
        ...
        this.axios.defaults.headers.common['x-loco-op-id'] =
          Config.OPERATION_ID;
