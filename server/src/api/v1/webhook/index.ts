...
router.put('/:rideId', async (req, res) => {
  ...
  const webhookRideData = req.body.data;

  const { stopPoints } = req.body.data;
  console.log(`Webhook - rideId: ${req.params.rideId} currentState: ${ride.state}  newState: ${req.body.data.state} SpsStates: ${(stopPoints.map(s => s.state)).join()}`);
  ...
  if (stopPoints && !ride.arrivingPush && (webhookRideData.state === 'active' || webhookRideData.state === 'dispatched')) {
  ...
  if (webhookRideData.state === 'active' || webhookRideData.state === 'dispatched') {
    ...
    ride.state = 'active';
    await ride.save();
  } else if (webhookRideData.state === 'completed') {
    ride.state = 'completed';
    await ride.save();
  } else if (webhookRideData.state === 'failed') {
    if (ride.state !== 'canceled') {
      cancelPush(ride.userId, ride.id, 'failureRide');
    }
    ride.state = 'canceled';
    await ride.save();
  } else if (webhookRideData.state === 'canceled') {
    ride.state = 'canceled';
    await ride.save();
    const currentRide = ride.get();
    if (!webhookRideData.cancellationReason.includes('user') && !currentRide.scheduledTo) {
      cancelPush(ride.userId, ride.id, 'findingNewDriver');

      const [pickup, dropoff] = webhookRideData.stopPoints;
      await rideService.create({
        userId: currentRide.userId,
        state: 'creating',
        numberOfPassengers: webhookRideData.numberOfPassengers,
        rideType: webhookRideData.rideType,
        scheduledTo: webhookRideData.scheduledTo,
        stopPoints: [
          {
            type: 'pickup',
    ...
    if (webhookRideData.cancellationReason.includes('no-show')) {
      cancelPush(ride.userId, ride.id, 'failureRide');
    }
  } else if (webhookRideData.state === 'rejected') {
    if (webhookRideData.scheduledTo) {
