...
const serializeRide = (ride) => {
  const serializedRide = {
    ...
    status: ride.state === 'offer-rejected' ? 'rejected' : ride.state,
    ...
    pickupTime: ride.offer && ride.offer.eta,
    dropoffEta: ride.offer && ride.offer.eta,
  };
...
const deserializeRide = (ride) => {
  const serializedRide = {
    ...
    rating: ride.rating,
    stopPoints: ride.offer_stop_points ?
      ride.offer_stop_points.map(sp => deserializeStopPoint(sp, ride.webhook_url)) :
      undefined,
...
module.exports = { serializeOffer: serializeRide, deserializeOffer: deserializeRide };
